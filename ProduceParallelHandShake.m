function [matout]=ProduceParallelHandShake(protein1,protein2,Persistence,nN,IncludePhos1, IncludePhos2)
...
pH=6.8;
[~, ~, ~, ~, ~, ~, seq1]=MCharge(protein1,pH,IncludePhos1,1); % do not limit segment (seq1,2) here or phospho calculation will be wrong. It must be the entire protein length
...
for NNorder=0:nN
    for i=1:len1
        for j=1:len2                    % Consider not calculating the whole matrix, but only half and then transpose!!! j=1:i           
            ...
            AAwindow2=FakeSeq2(j+nN:j+nN+2*Persistence);           
            ...
            if NNorder==0                                            % This is for no neighbors case. Cannot be deleted because the other cases are counted from both sides.
               ...
               %                 mat(i,j,NNorder+1)=horzcat(Annex,AAwindow1)*horzcat(AAwindow2,Annex)'+horzcat(AAwindow1,Annex)*horzcat(Annex,AAwindow2)';
                mat(i,j,NNorder+1)=horzcat(Annex1Left,AAwindow1)*horzcat(AAwindow2,Annex2Right)'+horzcat(AAwindow1,Annex1Right)*horzcat(Annex2Left,AAwindow2)';
            end
